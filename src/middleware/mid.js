...
// token error to be made 400 
catch(err){

    if(err.name ==='TokenExpiredError'){

        return res.status(403).send({status:false ,msg:"token toh expire hoh gayi"})
...
const authorise1=async function(req,res,next){
    ...
    if(err.name ==='JsonWebTokenError'){
        ...
        return res.status(403).send({status:false ,msg:"chahcha Eih galat hoh rhaa hai "})
